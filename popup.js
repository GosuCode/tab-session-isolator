document.addEventListener("DOMContentLoaded", async () => {
  ...
  function eyeIconSvg(crossedOut) {
    ...
    if (crossedOut) {
      svg.appendChild(svgEl("path", { d: "M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94" }));
      svg.appendChild(svgEl("path", { d: "M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19" }));
      svg.appendChild(svgEl("path", { d: "M14.12 14.12a3 3 0 1 1-4.24-4.24" }));
      svg.appendChild(svgEl("line", { x1: "1", y1: "1", x2: "23", y2: "23" }));
    } else {
      svg.appendChild(svgEl("path", { d: "M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" }));
  ...
  function deleteProfile(profile) {
    if (!confirm(`Delete profile "${profile.name}"? This closes its tabs and removes the saved password.`)) {
      return;
    }
    chrome.runtime.sendMessage({ action: "DELETE_PROFILE", profileId: profile.id }, (response) => {
      if (!response || !response.success) {
        reportError(response);
        return;
      }
      delete allProfiles[profile.id];
      renderProfiles();
    });
  }
  ...
  function renderProfiles() {
    ...
    Object.entries(grouped).forEach(([domain, items]) => {
      ...
      items.forEach((p) => {
        ...
        edit.appendChild(buildPencilIcon());
        ...
        del.addEventListener("click", () => deleteProfile(p));

        actions.append(edit, launch, del);
